...
An enterprise multi-format RAG architecture capable of ingesting PDF, Markdown, DOCX, and TXT files, providing cross-document synthesis and document-level metadata filtering.

---

## Key Features

- **Heterogeneous**: file format ingestion (PDF, Markdown, DOCX, TXT)
- **Metadata-tagged**: vector indexing with document type and author attributes
- **Cross-document**: comparative synthesis and multi-hop question answering
- **Hybrid**: lexical and dense vector search capabilities
- **Full**: REST API for document listing and grounded retrieval
...
```mermaid
flowchart LR
    PDF[PDF Files] --> Ingest[Multi-Format Ingestion Pipeline]
    MD[Markdown] --> Ingest
    DOCX[Word Docs] --> Ingest
    Ingest --> MetaVector[(Metadata Vector Store)]
    Query([Cross-Doc Query]) --> Engine[Multi-Doc RAG Engine]
    MetaVector --> Engine
    Engine --> Synthesis[Synthesized Multi-Doc Answer]
```
...
├── tests/
│   ├── __init__.py
│   └── test_multi_doc.py   # Automated test suite
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/multi-document-rag-system.git
...
| `POST` | `/query` | Query across all or specific indexed documents |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/query -H "Content-Type: application/json" -d '{"query": "Compare the SLA agreements across all vendors"}'
```
...
```bash
python -m unittest tests/test_multi_doc.py
```
